...
module.exports = ({ fromBranch, message = "Start your branch from:" }) => {
  ...
  if (
    startFromBranches.indexOf(toRemote(currentBranch)) === -1 &&
    git.branchExists(currentBranch, config.remote)
  ) {
  ...
  const remoteBranches = git
    ...
    .filter(
      name =>
        startFromBranches.indexOf(name) === -1 &&
        !(name === config.remoteDevelop || name === config.remoteProduction)
    );

  if (remoteBranches.length) {
    startFromBranches = startFromBranches.concat(
